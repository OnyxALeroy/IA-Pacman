...
for filename in files:
    ...
    if non_zero_fitness:
        ...
        avg_fitness.append(sum(non_zero_fitness) / len(non_zero_fitness))
    else:
        fitness_values.append(0)  # Append 0 if no non-zero fitness values are found
        avg_fitness.append(0)  # Append 0 for average fitness as well
...
# Calculate the moving average for avg_fitness (window size = 10)
avg_window_size = 10
avg_moving_average = np.convolve(avg_fitness, np.ones(avg_window_size)/avg_window_size, mode='valid')

# Calculate the trend line (linear regression) for avg_fitness
avg_generations = np.arange(len(avg_fitness))
avg_trend = np.polyfit(avg_generations, avg_fitness, 1)  # Linear regression (degree 1)
...
plt.plot(avg_generations, avg_fitness, marker="o", linestyle="-", color="purple", label="Average Best Fitness")

# Plot the moving average for avg_fitness
plt.plot(range(avg_window_size - 1, len(avg_fitness)), avg_moving_average, marker="o", linestyle="--", color="orange", label=f"Moving Average (window={avg_window_size})")

# Plot the trend line for avg_fitness
plt.plot(avg_generations, avg_trend_line, linestyle="--", color="green", label="Trend Line")

# Add titles and labels
plt.title("Average Fitness Evolution with Moving Average and Trend Line")
plt.xlabel("Generation")
...
plt.savefig("c:/Users/lowel/OneDrive/Documents/GitHub/IA-Pacman/Assets/Populations/avg_fitness_analysis.png", dpi=300, bbox_inches="tight")
plt.show()
